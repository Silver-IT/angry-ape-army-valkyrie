...
export default function suite() {
  ...
  it("should burn a token", async () => {
    ...
    await expect(ctx.valkyriesContract.connect(ctx.user1).burn(4)).to.emit(
    ...
    expect(await ctx.valkyriesContract.ownerOf(3)).to.be.eq(ctx.user1.address);
    await expect(ctx.valkyriesContract.ownerOf(4)).to.be.revertedWith(
      "OwnerQueryForNonexistentToken"
    );
    expect(await ctx.valkyriesContract.ownerOf(5)).to.be.eq(ctx.user1.address);
  });
  ...
  it("should fail to burn a token because it's called by the wrong user", async () => {
    ...
    await expect(
      ctx.valkyriesContract.connect(ctx.user2).burn(4)
  ...
  it("should burn a token because it's called by an approved user", async () => {
    ...
    ).to.emit(ctx.valkyriesContract, "ApprovalForAll");

    await expect(ctx.valkyriesContract.connect(ctx.approved).burn(4)).to.emit(
      ctx.valkyriesContract,
      "Transfer"
    );
  });
